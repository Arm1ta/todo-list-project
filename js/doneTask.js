const bottomDoneTask = document.querySelector('.bottom-doneTask');
const numberTaskDone = document.querySelector('.number-done-task');

function renderDoneList() {
    let toDoHTML = '';
    let sum = 0;
    doneTask.forEach((item,index) => {
        ...
        if (colorClass === 'بالا') colorClass = 'green';
         ...
         toDoHTML += `
            <div class="boxtask-bottom box-done">
                <div class="color-box ${colorClass}"></div>
                <div class="text-box" >
                    <div class='check-done' data-index-task = "${index}"></div>
    ...
    bottomDoneTask.innerHTML = toDoHTML
    numberTaskDone.innerHTML = sum;

}
renderDoneList()


const numberDoneTask = document.querySelector('.number-done-task');




bottomDoneTask.addEventListener('click', (e) => {
    if (e.target.closest('.recycleBin-svg')) {
        const indexTask = e.target.dataset.indexTask;
        doneTask.splice(indexTask, 1)
        renderDoneList();        
    }
